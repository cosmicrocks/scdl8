...
func (c *Controller) importConfigurationFromCRD(fromCRD *acidv1.OperatorConfigurationData) *config.Config {
	...
	result.DockerImage = util.Coalesce(fromCRD.DockerImage, "ghcr.io/cosmicrocks/spilo-15:3.0-p1")
	...
	result.PodServiceAccountName = util.Coalesce(fromCRD.Kubernetes.PodServiceAccountName, "scdl8-pod")
	...
	result.LogicalBackupDockerImage = util.Coalesce(fromCRD.LogicalBackup.DockerImage, "ghcr.io/cosmicrocks/logical-backup:v1.10.0")
	...
	result.PamRoleName = util.Coalesce(fromCRD.TeamsAPI.PamRoleName, "cosmics")
